        ...
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" method="POST" action="{{ route('product.edited')}}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" name="img" class="file-upload-default" id="uploadImage" onchange="PreviewImage();" value="{{$product->image}}" multiple>
                            <div class="input-group col-xs-12">
                              <input type="text" class="form-control file-upload-info" disabled="" value="{{$product->image}}" placeholder="upload product image">
                        ...
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <input type="text" class="form-control file-upload-info" value="{{$product->name}}" id="name" name="name" placeholder="type product name here.." />
                                @error('name') <span class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group col-6">
                                <label for="exampleInputName">Product Price</label>
                                <input type="text" class="form-control file-upload-info" value="{{$product->price}}" id="price" name="price" placeholder="₹" />
                        ...
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <select class="form-control file-upload-info" name="status" id="status" style="border:solid .1vmax #2a2c30;">
                                    <option value="instock" {{$product->status == 'instock' ? 'selected' : ''}}>Instock</option>
                                    <option value="outstock" {{$product->status == 'outstock' ? 'selected' : ''}}>OutStock</option>
                                  </select>
                                @error('status') <span class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group col-6">
                                <label for="exampleInputName">Product Quantity</label>
                                <input type="text" class="form-control file-upload-info" value="{{$product->quantity}}" id="quantity" name="quantity" placeholder="number of product" value="">
